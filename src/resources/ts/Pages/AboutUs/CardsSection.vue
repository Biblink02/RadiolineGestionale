...
import InspireCardButton from "@/Components/Custom/InspireCardButton.vue";
...
const cards = [
    {
        ...
        text: t('about-us.cards.2.button'),
...
<template>
    <div class="mx-auto gap-8 max-w-5xl flex flex-row flex-wrap justify-center place-content-center m-4 my-6">
        <InspireCard class="max-w-xs" v-for="card in cards" :card="card">
            <button class="group/button mt-auto mx-auto flex items-center gap-1 text-slate-500 hover:text-blue-600 transition-colors hover:cursor-pointer"
                    @click="card.onClick"
            >
                {{ card.text }}
    ...
    >
        <div v-show="visibleState !== 'none'" ref="contentRef" key="content"
             class="text-center px-4 mb-20 max-w-5xl mx-auto">
            <div v-if="visibleState === 'history'">
                <h2 class="text-3xl font-bold mt-4">{{ t('about-us.cards.history.title') }}</h2>
                <h3 class="text-xl font-semibold mt-4">{{ t('about-us.cards.history.subtitle') }}</h3>
            ...
            <div v-else-if="visibleState === 'mission'">
                <h2 class="text-3xl font-bold mt-4">{{ t('about-us.cards.mission.title') }}</h2>
                <h3 class="text-xl font-semibold mt-4">{{ t('about-us.cards.mission.subtitle') }}</h3>
                <p class="text-gray-600 leading-relaxed mt-3">
